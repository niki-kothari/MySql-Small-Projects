create schema music_store_niki;
use music_store_niki;

/* Easy questions */

-- ## Q1: Who is the senior most employee based on job title? */
select employee_id, concat(first_name, ' ', last_name) as Name, title from employee order by levels limit 1;
...
select c.customer_id, concat(first_name , ' ', last_name) as Name, sum(total) as 'Total Expense' from customer as c, invoice as i where c.customer_id = i.customer_id group by c.customer_id, Name order by 'Total Expense' desc limit 1;

/* Moderate Questions */

-- ## Q1: Write query to return the email, first name, last name, & Genre of all Rock Music listeners. Return your list ordered alphabetically by email starting with A.
select distinct email AS Email,concat (first_name, ' ', last_name) as 'Customer Name', genre.name AS 'Genre Name' from customer
...
select track_id, name, milliseconds from track where milliseconds > (select avg(milliseconds) from track) order by milliseconds desc;
